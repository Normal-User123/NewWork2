...
def create_referral(request: ReferralAddRequest, db: Annotated[Session,Depends(get_db)]):
    ...
    if not referrer or len(referrer.entitlements)<=0:
        raise HTTPException(status_code=503,detail=f"Invalid referral code: {request.referrer_code} or User Doesnot have any active plan!")

   
    referred_user = db.query(User).filter(User.id == request.referred_user_id).first()
    if not referred_user:
        raise HTTPException(status_code=503,detail=f"Invalid referred user ID: {request.referred_user_id}")
