...
def check_for_transaction(user_id, start_time, admin_wallet_id, stop_event,transaction_id):
    ...
    with next(get_db()) as db:
        while not stop_event.is_set():
            try:
                url1 = f"https://api.bscscan.com/api?module=account&action=tokentx&address={admin_wallet_id}&page=1&offset=20&sort=desc&apikey={settings.apikey}"
                print(url1)
                response = requests.get(str(url1))
